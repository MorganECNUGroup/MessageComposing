...
export class ChatMessagePage {
  ...
  public showCompletionCandidates(ev){
    ...
    if (this.chatContent&&this.chatContent.charAt(this.chatContent.length-1)==' '&&this.chatContent.replace(/\s*/g,"")!=''){    
      ...
      //console.log("以空格结尾");
      this.getMessageService.sendMessage("( Incomplete Message ) "+new Date().toString()+":  "+this.chatContent);
    ...
    if (message && message !== '') {
     ...
     // this.messages.push();
      ...
      setTimeout(() => {
        ...
        this.getMessageService.sendMessage(replyData.date+" "+replyData.username+":  "+replyData.text);
      }, 1000);
      ...
      this.getMessageService.sendMessage(messageData.date+" "+messageData.username+":  "+messageData.text);
    }
    this.chatContent = '';
    this.isShowCompletionCandidates=false;
   // this.composingChatBox='';
